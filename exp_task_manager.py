...
def start_up(prompts):
    ...
    while timeout < 10:
        ...
        if init_conditions_satisfied == 6:      # it should be 3 but got 6
            play_audio_file(f"audio/voice_{counter}.mp3")
            # play_audio_file(f"audio/voice_8.mp3")
            print("!!!!!!!FINISH ORGANA AUDIO!!!!!!!!!!!!!!")
            if AUDIO_MODE:
                pass
            print(
        ...
        if AUDIO_MODE:
            ...
            # for testing
            input_usr=[
                "I would like to generate a pourbaix plot for an unknown quinone",
                "I have a solution containing 10mM of the quinone, water, 1M sodium chloride solution, and a series of 0.5 M pH X buffer solution (X is an integer between 7 to 9), two beakers (experiment beaker, waste beaker), a pH probe and potentiostat.",
                ...
                "Add 6 mL of pH 7 buffer solution. Add 3 ml NaCl solution. add 15 mL of water followed by 6 mL of quinone solution. Then measure the pH and run a CV scan. transfer r contents of beaker to waste beaker.",
...
class TaskManager:
    ...
    def nlp_postprocess(self):
        ...
        if DEBUG_MODE:
            ...
            #     "what was the ph!ev!constraints from the experiment? "
            # ).split("!")  
            ...
            # obs_list = [[{'pH': 4, 'eV': -0.38}],
            #             [{'pH': 4, 'eV': -0.38}, {'pH': 5, 'eV': -0.47}], 
            #             [{'pH': 4, 'eV': -0.38}, {'pH': 5, 'eV': -0.47}, {'pH': 6, 'eV': -0.67}]]
            #observation = obs_list[self.step]
            system_feedback = ""
            human_feedback = "" 
